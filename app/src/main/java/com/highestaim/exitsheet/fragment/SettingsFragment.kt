...
class SettingsFragment : BaseFragment() {
    ...
    private fun setChangeLanguageDialogButtonsClickListener(languageChangeDialog: AlertDialog.Builder?) {
        languageChangeDialog?.setPositiveButton(resources.getString(R.string.ok)) { _, _ ->
            when (selectedLanguageIndex) {
                0 -> changeLanguage(LanguagesEnum.ENGLISH.language)
                1 -> changeLanguage(LanguagesEnum.ARMENIAN.language)
                2 -> changeLanguage(LanguagesEnum.RUSSIAN.language)
            }
        }
    ...
    private fun changeLanguage(languageCode: String?) {
        context?.let { languageCode?.let { it1 -> LocaleManager.setNewLocale(it, it1) } }
        startActivity(Intent.makeRestartActivityTask(Intent(activity, MainActivity::class.java).component))
    }
